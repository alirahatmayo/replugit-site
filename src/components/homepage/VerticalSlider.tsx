    ...
    <section ref={sectionRef} className="pt-12 sm:pt-16 lg:pt-20 pb-12 sm:pb-16 lg:pb-20 bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          {/* Mobile Vertical Card Deck Stack */}
          <div className="md:hidden">
            {/* Card Deck Container */}
            <div className="h-[650px] overflow-y-auto snap-y snap-mandatory scrollbar-hide relative">
              {solutions.map((solution, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="h-full snap-start snap-always flex items-center p-2"
                    style={{
                      transform: `translateY(${index * -2}px)`,
                      zIndex: solutions.length - index
                    }}
                  >
                    <div className={`w-full p-6 rounded-2xl border-l-4 ${getAccentColor(solution.color)} shadow-lg hover:shadow-xl transition-all duration-500 ease-in-out transform hover:scale-[1.02] relative overflow-hidden`}>
                        ...
                        <div className="mb-6">
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {solution.features.map((feature, featureIndex) => (
                              <div key={featureIndex} className="flex items-center gap-3 py-1">
              ...
              {solutions.map((solution, index) => (
                <div
                  ...
                  className={`w-2 h-2 rounded-full transition-all duration-300 ${
                    solution.color === 'blue' ? 'bg-blue-300' :
                    solution.color === 'green' ? 'bg-green-300' :
                    solution.color === 'purple' ? 'bg-purple-300' :
                    solution.color === 'red' ? 'bg-red-300' :
                    'bg-emerald-300'
                  }`}
                />
              ))}
            ...
            <div className="text-center mt-4">
              <p className="text-slate-500 text-xs font-medium">↕ Scroll through our solutions</p>
            </div>
          </div>
